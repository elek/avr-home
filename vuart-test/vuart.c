...
#define configure_external_interrupt()  EICRA = (EICRA & ~(1 << ISC01 | 1 << ISC01)) | (1 << ISC01)
...
/*
 * vuart_init() configures external and timer interrupt and enables external interrupt
 */
void vuart_init()
{
  // configuring and putting RX and TX pins into their initial state
#if VUART_RECEIVE_SUPPORT
 ...
 * If non-blocking behaviour is needed, check vuart_is_receive_complete()
